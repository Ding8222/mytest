...
bool CClientMgr::Init(int32 maxclientnum, int32 listenport, int32 clientovertime, int32 recvdatalimit, int32 senddatalimit)
{
...
{
	snprintf(buf, buflen - 1, "最大 Client 限制:%d\n当前 Client 数量:%d\n总ID数量 total:%d\n当前使用ID数量:%d\n待移除 Client 数量:%d\n", m_MaxClientNum, (int32)m_ClientList.size(), idmgr_total(m_IDPool), idmgr_usednum(m_IDPool), (int32)m_WaitRemove.size());
}

CClient *CClientMgr::FindClient(int32 clientid)
{
	if (clientid <= 0 || clientid >= (int32)m_ClientSet.size())
		return NULL;
...
void CClientMgr::AsLogicServerDisconnect(int32 logicserverid)
{
	for (std::list<CClient*>::iterator itr = m_ClientList.begin(); itr != m_ClientList.end(); ++itr)
	{
...
{
	if (clientid <= 0 || clientid >= static_cast<int32>(m_ClientSet.size()))
		return nullptr;
...
void CClientMgr::ProcessAllClient()
{
	...
	for (itr = m_ClientList.begin(); itr != m_ClientList.end();)
	{
		...
		{
			//ClientConnectLog("服务器延时移除Client, ID:%d, 剩余Client数量:%d", (*tempitr)->GetClientID(), (int32)m_ClientList.size() - 1);
		...
		{
			log_writelog("与Client的连接断开, id:%d, 剩余Client数量:%d", (*tempitr)->GetClientID(), (int32)m_ClientList.size() - 1);
		...
		{
			log_writelog("与Client的连接超时关闭, id:%d, 剩余Client数量:%d", (*tempitr)->GetClientID(), (int32)m_ClientList.size() - 1);
			(*tempitr)->GetCon()->Close();
			OnClientDisconnect(*tempitr);
			m_ClientList.erase(tempitr);
			continue;
