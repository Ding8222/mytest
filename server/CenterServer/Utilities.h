...
namespace FuncUti
{
	...
	{
		CCentServerMgr::Instance().SendMsgToServer(pMsg, maintype, subtype, ServerEnum::EST_GAME, playerguid, nServerID, bBroad);
	}
	inline void SendMsgToGame(int64 playerguid, Msg &pMsg, int nServerID = 0, bool bBroad = false)
	{
		CCentServerMgr::Instance().SendMsgToServer(pMsg, ServerEnum::EST_GAME, playerguid, nServerID, bBroad);
	}
}